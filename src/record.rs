    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            ParseError::InvalidFasta(record) => {
                write!(f, "Not a validate fasta record: {}", record)
            }
            ParseError::InvalidFastq(record) => {
                write!(f, "Not a validate fastq record: {}", record)
            }
        }
    }
}
...
impl Fastx<'_> {
    ...
    pub fn head(&self) -> &str {
        unsafe { str::from_utf8_unchecked(&self._data[1..self._head - 1]) }
    }

    /// get sequence
    #[inline]
    pub fn seq(&self) -> &str {
        unsafe { str::from_utf8_unchecked(&self._data[self._des..self._seq - 1]) }
    }
    ...
    pub fn des(&self) -> &str {
        if self._des != self._head {
            unsafe { str::from_utf8_unchecked(&self._data[self._head..self._des - 1]) }
        } else {
            ""
        }
    }
    ...
    pub fn sep(&self) -> &str {
        if self._seq != self._sep {
            unsafe { str::from_utf8_unchecked(&self._data[self._seq..self._sep - 1]) }
        } else {
            ""
        }
    }
    ...
    pub fn qual(&self) -> &str {
        if self._sep != self._qual {
            unsafe { str::from_utf8_unchecked(&self._data[self._sep..self._qual - 1]) }
        } else {
            ""
        }
    }
    ...
    fn validate_fastq(&self) -> bool {
        self._seq - self._des == self._qual - self._sep && self._data[0] == b'@' && !self.is_empty()
    }

    /// check a fasta record is valid
    fn validate_fasta(&self) -> bool {
        self._data[0] == b'>' && !self.is_empty()
    }
}
...
impl Reader {
    ...
    fn reach_eof(&mut self) -> Result<bool> {
        loop {
            ...
            if !char::is_whitespace(self.data[0] as char) {
                break;
            }
        }
        Ok(false)
